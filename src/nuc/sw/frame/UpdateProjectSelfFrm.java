...
public class UpdateProjectSelfFrm extends JInternalFrame {
...
//	}
	...
	public UpdateProjectSelfFrm() {
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				updateProjectSelfActionPerformed(e);
			}
		});
	 ...
	 * @param e
	 */
	private void updateProjectSelfActionPerformed(ActionEvent e) {
		String id = this.IdTxt.getText();
		if(StringUtil.isEmpty(id)) {
			JOptionPane.showMessageDialog(null, "请选择要修改的课题。");
			return;
		}
		String projectSelfName = this.SelfNameTxt.getText();
		String studentId = this.StudentIdTxt2.getText();
		String summary = this.DescTxt.getText();
		...
		ProjectType projectType = (ProjectType)this.Jcb.getSelectedItem();
		int projectTypeId = projectType.getId();
		
		ProjectSelf projectSelf = new ProjectSelf(Integer.parseInt(id), projectSelfName, summary, projectTypeId, studentId);
		
		Connection con = null;
		try {
			con = myjdbc.getCon();
			int updateNum = projectSelfDao.update(con, projectSelf);
			if(updateNum==1) {
				JOptionPane.showMessageDialog(null, " 修改成功！");
				
				String studentId1 = this.StudentIdTxt.getText();
				ProjectSelf projectSelf1 = new ProjectSelf(studentId1);
				this.fillTable(projectSelf1);
				resetValue();
			}else {
